...
class GLMNB(torch.nn.Module):
    def __init__(self, beta_dim=None, gamma_dim=None, covariates=False, penalty='No', n_study=None, device="cpu"):
        super().__init__()
        self.covariates = covariates
        self.penalty = penalty
        ...
        alpha_init = torch.tensor([0.1], dtype=torch.float64, device=self.device)
    ...
    def first_three_term(y, r, device):
        max_foci = np.int64(torch.max(y).item())
        sum_three_term = 0
        for k in range(max_foci):
            foci_index = (y == k+1).nonzero()[:,0]
            r_j = r[foci_index]
            n_voxel = list(foci_index.shape)[0]
            y_j = torch.tensor([k+1]*n_voxel, dtype=torch.float64, device=device)
    ...
    def _log_likelihood(alpha, beta, gamma, X, y, Z, y_t, device):
        n_study = y_t.shape[0]
        n_voxel = X.shape[0]
        v = 1 / alpha
        # mu_X
        ...
        l = GLMNB.first_three_term(y,r, device) + torch.sum(r*torch.log(1-p) + y*torch.log(p))
    ...
    def forward(self, X, y, Z=None, y_t=None):
        ...
        if self.covariates == True:
            ...
            log_mu_Z = torch.zeros(self.n_study, 1, device=self.device)
            mu_Z = torch.ones(self.n_study, 1, device=self.device)
        ...
        l = GLMNB.first_three_term(y,r,self.device) + torch.sum(r*torch.log(1-p) + y*torch.log(p))
...
class GLMClustered_NB(torch.nn.Module):
    def __init__(self, beta_dim=None, gamma_dim=None, covariates=False, penalty='No', device="cpu"):
        ...
        torch.nn.init.uniform_(self.beta_linear.weight, a=-0.01, b=0.01)
        # initialization for alpha: uniform distribution between 0 and 1
        alpha_init = torch.tensor(1e-2, dtype=torch.float64, device=self.device)
...
class GLMZIP(torch.nn.Module):
    def __init__(self, beta_dim=None, gamma_dim=None, covariates=False, penalty='No', device='cpu'):
        ...
        torch.nn.init.uniform_(self.gamma_linear.weight, a=-0.01, b=0.01)
        # psi
        psi_init = torch.tensor(0.01, dtype=torch.float64, device=self.device)
    ...
    def forward(self, X, y, Z, y_t, y_p):
        ...
        for i in range(n_study):
            ...
            zero_voxel = GLMZIP.setdiff(torch.arange(N,device=self.device), nonzero_voxel)
...
class GLMQPOI(torch.nn.Module):
    def __init__(self, beta_dim=None, gamma_dim=None, covariates=False, penalty='No', device="cpu"):
        ...
        if self.covariates == True:
            ...
            self.gamma_dim = None
        # phi
        x_init = torch.tensor(0, dtype=torch.float64, device=self.device)
