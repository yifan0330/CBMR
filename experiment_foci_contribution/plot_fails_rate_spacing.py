...
for group in groups:
    ...
    for dset in dataset_group:
        ...
        spline_spacings = sorted(spline_spacings, reverse=False)
        marker_size = dataset_n_foci_increasing_index[dset_index]
        print(group, dset)
        print(dset_fail_rate)
        print(spline_spacings)
        axs[row_idx, col_idx].scatter(spline_spacings, dset_fail_rate, label=f"{dset} (scatter)", s=marker_size)
        axs[row_idx, col_idx].plot(spline_spacings, dset_fail_rate)
        axs[row_idx, col_idx].set_xlabel("Spline spacings", fontsize=30)
...
fig.savefig("plot_spline_spacing_fail_rates.pdf")
