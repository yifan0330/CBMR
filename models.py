...
class GLMNB(torch.nn.Module):
    def __init__(self, beta_dim=None, gamma_dim=None, covariates=False, penalty='No'):
        ...
        alpha_init = torch.tensor([0.0], dtype=torch.float64, device='cuda')
        # alpha = softplus(theta) + 1e-8
        self.theta = torch.nn.Parameter(alpha_init, requires_grad=True).double()
        if self.covariates == True:
            self.gamma_dim = gamma_dim
            self.gamma_linear = torch.nn.Linear(self.gamma_dim, 1, bias=False).double()
    ...
    def _log_likelihood(alpha, beta, gamma, X, y, Z, y_t):
        ...
        # expression = torch.lgamma(y+r) - torch.lgamma(y+1) - torch.lgamma(r) + r*torch.log(1-p) + y*torch.log(p)
        # l = torch.sum(expression)
        # print(l)
        # mu_array = GLMNB.kron(mu_Z, mu_X)
        # p = torch.sum(mu_array**2, dim=0) / (v*torch.sum(mu_array, dim=0) + torch.sum(mu_array**2, dim=0))
        # r = v * torch.sum(mu_array, dim=0)**2 / torch.sum(mu_array**2, dim=0)
        # expression = torch.lgamma(y+r) - torch.lgamma(y+1) - torch.lgamma(r) + r*torch.log(1-p) + y*torch.log(p)
        # l = torch.sum(expression)
        # print(l)
        # exit()
    ...
    def forward(self, X, y, Z=None, y_t=None):
        ...
        self.alpha = torch.nn.Softplus()(self.theta) + 1e-8
        # sum_i mu_{ij}^2 = sum_i (mu^Z_i)^2 * (mu^X_j)^2 = (mu^X_j)^2 * sum_i (mu^Z_i)^2
        ...
        self.estimated_alpha = torch.clamp(self.estimated_alpha, max=100)
        # print(self.estimated_alpha.item())
        # print(self.estimated_alpha.item(), torch.min(self.beta_linear.weight).item(), torch.max(self.beta_linear.weight).item())
...
class GLMClustered_NB(torch.nn.Module):
    ...
    def forward(self, X, y, Y, Z, y_t, y_p):
        v = 1 / self.alpha
        # mu^X = exp(X * beta)
        log_mu_X = self.beta_linear(X) 
        mu_X = torch.exp(log_mu_X)
