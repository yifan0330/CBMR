...
class train_dataset(object):
    def __init__(self, homogeneous, intensity, covariates, spline_param, spacing, device='cpu'):
        self.device = device
    ...
    def generate_data(self, n_study, x_range=100, y_range=100, r=42):
        np.random.seed(r)
        # X
        if self.spline_param: 
            X_filename = os.getcwd() + "/Spline_bases_matrix/X_spacing{}.npy".format(str(self.spacing))
            if not os.path.isfile(X_filename):
                X = self.B_spline_bases(x_range, y_range, self.spacing)
                np.save(X_filename, X)
                print("create spline parameterised X matrix")
            else: 
                X = np.load(X_filename)
                print("load spline parameterised X matrix")
        else:
            X_filename = os.getcwd() + "/Gaussian_kernel_matrix/X_spacing{}.npy".format(str(self.spacing))
            if not os.path.isfile(X_filename):
                X = self.Gaussian_kernel_basis(x_range, y_range, self.spacing)
                np.save(X_filename, X)
                print("create X matrix from gaussian kernels")
            else:
                X = np.load(X_filename)
                print("load X matrix from gaussian kernels")
        self.X = torch.tensor(X, dtype=torch.float64, device=self.device)
        
        # y and y_t
        self.y = np.zeros(shape=(x_range*y_range, 1))
        self.y_t = np.empty(shape=(0,1))
        
        if self.homogeneous:
            intensity_array = self.intensity * np.ones((x_range, y_range))
        else:
            ...
            for center in Gaussian_centers:
                x_0, y_0 = center
                gaussian_intensity = self.gaussian_2d(X+0.5, Y+0.5, x_0, y_0, sigma) - self.gaussian_2d(X-0.5, Y+0.5, x_0, y_0, sigma) \
                                    - self.gaussian_2d(X+0.5, Y-0.5, x_0, y_0, sigma) + self.gaussian_2d(X-0.5, Y-0.5, x_0, y_0, sigma)
                intensity_array += self.intensity * gaussian_intensity
        ...
        for i in range(n_study):
            ...
            self.y += y_i.reshape((-1,1))
            self.y_t = np.concatenate((y_t_i, self.y_t), axis=0)
        ...
        # Z
        self.Z = np.random.uniform(-0.01, 0.01, (n_study, 2))
    ...
    def B_spline_bases(self, x_range, y_range, spacing, margin=20):
        self.margin = margin
        xx = np.arange(0, x_range)
        yy = np.arange(0, y_range)
        ...
        self.n_x_spline_bases, self.n_y_spline_bases = x_spline.shape[1], y_spline.shape[1]
        # create spatial design matrix by tensor product of spline bases in 2 dimesions
        X = np.kron(x_spline, y_spline) # Row sums of X are all 1=> There is no need to re-normalise X
        ...
        for bx in range(self.n_x_spline_bases):
            for by in range(self.n_y_spline_bases):
                basis_index = by + self.n_y_spline_bases * bx
    ...
    def gaussian_kernel(self, X, xx_centers, sigma):
        X = X.astype('float64')
        X_gaussian_kernel = [np.exp(-(X-center)**2 / (2 * sigma**2)).reshape((-1,1)) for center in xx_centers]
        X_gaussian_kernel = np.concatenate(X_gaussian_kernel, axis=1)
        
        return X_gaussian_kernel

    def Gaussian_kernel_basis(self, x_range, y_range, spacing, margin=20):
        self.sigma = spacing
        self.margin = margin
        xx = np.arange(0, x_range)
        yy = np.arange(0, y_range)
        ...
        wider_xx = np.arange(np.min(xx) - margin, np.max(xx) + margin)
        xx_centers = np.arange(np.min(wider_xx), np.max(wider_xx), step=spacing)
        x_gaussian_kernel = self.gaussian_kernel(wider_xx, xx_centers, spacing)
        x_gaussian_kernel = x_gaussian_kernel[margin : -margin + 1, :]
        
        wider_yy = np.arange(np.min(yy) - margin, np.max(yy) + margin)
        yy_centers = np.arange(np.min(wider_yy), np.max(wider_yy), step=spacing)
        print(xx_centers)
        print(yy_centers)
        exit()
        y_gaussian_kernel = self.gaussian_kernel(wider_yy, yy_centers, spacing)
        y_gaussian_kernel = y_gaussian_kernel[margin : -margin + 1, :]
        # kronecker product
        X = np.kron(x_gaussian_kernel, y_gaussian_kernel)
        
        self.n_x_gaussian_bases, self.n_y_gaussian_bases = x_gaussian_kernel.shape[1], y_gaussian_kernel.shape[1]
        # remove tensor product basis that have no support in the brain
        support_basis = []
        # find and remove weakly supported B-spline bases
        for bx in range(self.n_x_gaussian_bases):
            for by in range(self.n_y_gaussian_bases):
                basis_index = by + self.n_y_gaussian_bases * bx
                basis_coef = X[:, basis_index]
                if np.max(basis_coef) >= 0.1: 
                    support_basis.append(basis_index)
        X = X[:, support_basis]
    ...
    def gaussian_2d(self, x, y, x0, y0, sigma):
    ...
    def mkdir(self):
        if self.covariates:
            foldername_1 = "with_covariate_results/"
        else:
            foldername_1 = "no_covariate_results/"
        if self.homogeneous:
            foldername_2 = "Homogeneous_"
        else:
            foldername_2 = "Gaussian_signal_"
        if self.spline_param:
            foldername_3 = "Spline_parameterisation/"
        else:
            foldername_3 = "Gaussian_kernel/"
        PATH = os.getcwd() + '/results/with_covariate_results/' \
            + foldername_2 + str(self.intensity) + '/' + self.model \
                + '_model/' + self.penalty + '_penalty/' + foldername_3
        # make directory if it doesn't exist
        if not os.path.exists(PATH):
            os.makedirs(PATH)
        return PATH

    def train(self, model, penalty, n_experiment, iter=1500, lr=0.01, tol=1e-4):
        self.model = model
        ...
        for i in range(self.n_experiment):
            ...
            self.generate_data(n_study=100, x_range=100, y_range=100, r=i)
        ...
        all_beta = np.concatenate(all_beta, axis=1)
        all_gamma = np.concatenate(all_gamma, axis=1)
        # save to files
        PATH = self.mkdir()
        print(PATH)
        np.save(PATH+'beta.npy', all_beta)
        np.save(PATH+'gamma.npy', all_gamma)
